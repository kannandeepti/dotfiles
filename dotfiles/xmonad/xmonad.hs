...
import Control.Monad    -- used only for confirm
import System.IO
import System.Exit
import XMonad
...
------------------------------------------------------------------------
-- Confirmer
-- A function used to present a confirmation on quitting xmonad with
-- dmenu. See M-s-q and M-q keybindings for example usage.
--
confirm :: String -> X () -> X ()
confirm m f = do
  result <- dmenu [m, "y","n"]
  when (head result == 'y' || head result == 'Y') f
...
myKeys conf@(XConfig {XMonad.modMask = modMask}) = M.fromList $
  ...
  , ((modMask, xK_p),
     spawn "dmenu_run | dmenu")

  -- Take a screenshot in select mode.
  -- After pressing this key binding, click a window, or draw a rectangle with
  -- the mouse.
  --DEPRECATED - this key now used to play/pause audio
  ...
  , ((modMask .|. controlMask, xK_m),
     spawn "amixer -c 1 set Master 0%")

  -- Decrease mpd volume.
  , ((modMask .|. controlMask, xK_j),
     spawn "ncmpcpp volume -10")
  ...
  , ((0, 0x1008FF11),
     spawn "amixer -c 1 set Master 10%-")
...
myMouseBindings (XConfig {XMonad.modMask = modMask}) = M.fromList $
  [
    ...
    -- mod-button2, (scroll wheel click) - Raise the window to the top of the stack
    , ((modMask, button2),
